...
## Task 6: Epic 스펙 업데이트
- [x] epic-05 가격 전체 업데이트 (Hardcore 포함)
- [x] epic-09 잭팟 시스템 전면 개편 (동적 풀, HP:1500, 이탈 처리)
- [x] epic-15 시스템 곡괭이 어댑티브 (휴식/활성 모드, 최대 4개, 100명 테이블)
- [x] epic-16 신규 생성 (하우스수익-잭팟풀 메커니즘)
- [x] 기능명세체크리스트.md 업데이트
- [x] 커밋

## Task 7: 코드 리뷰
- [x] superpowers:requesting-code-review 호출
- [x] 피드백 반영 (5개 버그 수정: 앵커 모드전환 크기동기화, system_weak 보안, 잭팟 이중지급, 시뮬레이터 모델 오차, 레거시 필드 제거)
- [x] 최종 완료
