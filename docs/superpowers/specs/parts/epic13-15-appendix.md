...
**[STORY 13-1]** 유저는 내 정보를 보기 위해 화면 상단의 '내 정보' 버튼을 클릭할 수 있어야 한다.
...
**[STORY 13-2]** 유저는 팝업 내 X 버튼을 클릭하여 내 정보 팝업을 닫을 수 있어야 한다.
...
**[STORY 13-3]** 유저는 팝업 외부 영역(백드롭)을 클릭하여 내 정보 팝업을 닫을 수 있어야 한다.
...
**[STORY 13-4]** 미로그인 상태에서 '내 정보' 버튼을 클릭할 경우 시스템이 지갑 연결을 유도해야 한다.
...
**[STORY 13-5]** 유저는 Overview 탭에서 잔액, 총 획득, 총 소비, PnL을 확인할 수 있어야 한다.
...
**[STORY 13-6]** PnL이 양수(수익)일 때 녹색으로 표시되어야 한다.
...
**[STORY 13-7]** PnL이 음수(손실)일 때 빨간색으로 표시되어야 한다.
...
**[STORY 13-8]** 유저는 내 정보 팝업 내 '무료 크레딧 충전' 버튼을 클릭하여 크레딧을 충전할 수 있어야 한다.
...
**[STORY 13-9]** 유저는 내 정보 팝업 내 '로그아웃' 버튼을 클릭하면 확인 팝업을 볼 수 있어야 한다.
...
**[STORY 13-10]** 유저가 로그아웃 확인 다이얼로그에서 '확인'을 클릭하면 로그아웃이 완료되어야 한다.
...
**[STORY 13-11]** 유저가 로그아웃 확인 다이얼로그에서 '취소'를 클릭하면 로그아웃이 취소되어야 한다.
...
**[STORY 13-12]** 유저는 내 정보 팝업 내 탭을 클릭하여 'Overview' / '지갑 연결(Deposit/Withdraw)' 탭을 전환할 수 있어야 한다.
...
**[STORY 13-13]** 닉네임 로그인 유저는 입출금 탭(Deposit/Withdraw)을 이용할 수 없어야 한다.
...
**[STORY 14-1]** 유저는 Deposit 탭을 열어 입금 UI를 확인할 수 있어야 한다.
...
**[STORY 14-3]** 금액 입력 시 예상 수령 크레딧이 실시간으로 표시되어야 한다.
...
**[STORY 14-4]** 유저는 + / - 버튼으로 입금 금액을 1 USDC 단위로 조정할 수 있어야 한다.
...
**[STORY 14-5]** 유저가 입금 버튼을 클릭하면 1단계 USDC Approve 트랜잭션이 요청되어야 한다.
...
**[STORY 14-6]** Approve 트랜잭션 서명 후 '승인 중...' 상태가 표시되어야 한다.
...
**[STORY 14-7]** Approve 온체인 확인이 완료되면 2단계 Deposit 트랜잭션이 요청되어야 한다.
...
**[STORY 14-8]** Deposit 트랜잭션 서명 후 '입금 중...' 상태가 표시되어야 한다.
...
**[STORY 14-9]** 서버가 온체인 Deposit 이벤트를 확인하면 크레딧이 지급되어야 한다.
...
**[STORY 14-11]** Approve 단계에서 서명을 거부하면 오류 메시지가 표시되어야 한다.
...
**[STORY 14-12]** Deposit 단계에서 서명을 거부하면 오류 메시지가 표시되어야 한다.
...
**[STORY 14-13]** 입출금 진행 중 버튼이 비활성화되어야 한다.
...
**[STORY 14-14]** 유저는 Withdraw 탭을 열어 출금 UI를 확인할 수 있어야 한다.
...
**[STORY 14-15]** 유저는 출금 금액을 직접 입력할 수 있어야 한다.
...
**[STORY 14-16]** 금액 입력 시 차감될 크레딧이 실시간으로 표시되어야 한다.
...
**[STORY 14-17]** 유저는 + / - 버튼으로 출금 금액을 1 USDC 단위로 조정할 수 있어야 한다.
...
**[STORY 14-18]** 유저가 출금 버튼을 클릭하면 Vault Withdraw 트랜잭션이 요청되어야 한다.
...
**[STORY 14-19]** 출금 트랜잭션 서명 후 '출금 중...' 상태가 표시되어야 한다.
...
**[STORY 14-20]** 서버가 온체인 Withdraw 이벤트를 확인하면 크레딧이 차감되어야 한다.
...
**[STORY 14-21]** 출금 완료 후 크레딧이 즉시 차감 반영되어야 한다.
...
**[STORY 14-22]** 잔액이 부족할 경우 출금 버튼 클릭 시 오류 메시지가 표시되어야 한다.
...
**[STORY 14-23]** 출금 트랜잭션 거부 시 오류 메시지가 표시되고 크레딧은 차감되지 않아야 한다.
...
**[STORY 14-24]** 지갑 미연결 상태에서 입출금을 시도할 경우 오류 메시지가 표시되어야 한다.
...
**[STORY 14-25]** Vault 컨트랙트가 미배포 상태일 경우 오류 메시지가 표시되어야 한다.
...
**[STORY 14-26]** 소수점 금액 입력은 자동으로 정수로 강제 변환되어야 한다.
...
**[STORY 14-27]** 동일한 트랜잭션 해시를 재사용하여 크레딧을 중복으로 획득/차감할 수 없어야 한다.
...
**[STORY 15-1]** 게임 시작 시 PIKIT 곡괭이가 자동으로 필드에 배치되어야 한다.
...
**[STORY 15-2]** PIKIT 곡괭이가 블록에 접촉하면 블록을 파괴하고 보상을 수취해야 한다.
...
**[STORY 15-3]** PIKIT 곡괭이는 숍에 표시되지 않아야 한다.
...
**[STORY 15-4]** PIKIT 곡괭이는 플레이어 곡괭이와 시각적으로 구별되어야 한다.
...
**[STORY 15-5]** PIKIT 곡괭이가 필드에서 제거된 후 5초 이내에 재생성되어야 한다.
...
**[STORY 15-6]** 플레이어 수가 증가할수록 PIKIT 곡괭이가 블록을 가져가는 비율(스틸 레이트)이 감소해야 한다.
