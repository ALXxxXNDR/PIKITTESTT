...
곡괭이가 블록을 파괴할 때마다 연속 파괴 횟수(콤보)를 추적하여, 연속성이 높을수록 보상 배율을 증가시키는 시스템이다. 플레이어가 곡괭이를 적극적으로 운용하도록 유도하고, 단순 방치보다 집중 플레이를 보상한다. 콤보는 곡괭이 단위로 독립적으로 계산되며, 마지막 블록 파괴 후 2,000ms(2초) 이내에 다음 블록을 파괴하지 못하면 초기화된다.

---

### ❖ 2. User Story & 상세 플로우

- **[STORY 8-1]** 유저는 곡괭이가 블록을 처음 파괴할 때 콤보 없이 기본 배율(1.0x)을 적용받아야 한다.
  1. 곡괭이가 블록을 파괴한다.
  2. 해당 곡괭이의 `lastHitTime`이 존재하지 않거나 현재 시각과의 차이가 2,000ms를 초과한다.
  3. 콤보 카운터가 1로 설정된다.
  4. COMBO.THRESHOLDS = [0, 3, 6, 10, 15, 25] 기준으로 콤보 1은 인덱스 0에 해당 → 배율 1.0x 적용.
  5. 최종 보상 = 블록 기본 보상 × 1.0 × 필드 배율.

- **[STORY 8-2]** 유저는 2초 이내 연속 타격 3회 달성 시 콤보 배율 1.05x를 받아야 한다.
  1. 곡괭이가 연속으로 3번째 블록을 파괴한다. 각 파괴 간격이 2,000ms 미만이다.
  2. 콤보 카운터가 3으로 도달한다.
  3. THRESHOLDS에서 3 이상인 구간(인덱스 1) 적용 → 배율 1.05x.
  4. 예시: 석재(28cr) 파괴 시 → 28 × 1.05 = 29.4 → 반올림 = 29cr.

- **[STORY 8-3]** 유저는 연속 타격 6회 달성 시 콤보 배율 1.1x를 받아야 한다.
  1. 콤보 카운터가 6 이상이 된다.
  2. THRESHOLDS 인덱스 2 적용 → 배율 1.1x.
  3. 예시: 석재(28cr) → 28 × 1.1 = 30.8 → 31cr.

- **[STORY 8-4]** 유저는 연속 타격 10회 달성 시 콤보 배율 1.2x를 받아야 한다.
  1. 콤보 카운터가 10 이상이 된다.
  2. THRESHOLDS 인덱스 3 적용 → 배율 1.2x.
  3. 예시: 구리 블록(50cr) → 50 × 1.2 = 60cr.

- **[STORY 8-5]** 유저는 연속 타격 15회 달성 시 콤보 배율 1.35x를 받아야 한다.
  1. 콤보 카운터가 15 이상이 된다.
  2. THRESHOLDS 인덱스 4 적용 → 배율 1.35x.
  3. 예시: 에메랄드(540cr) → 540 × 1.35 = 729cr.

- **[STORY 8-6]** 유저는 연속 타격 25회 이상 달성 시 최대 콤보 배율 1.5x를 받아야 한다.
  1. 콤보 카운터가 25 이상이 된다.
  2. THRESHOLDS 인덱스 5 적용 → 배율 1.5x (최대값, 이후 상승 없음).
  3. 예시: 철 블록(100cr) → 100 × 1.5 = 150cr.

- **[STORY 8-7]** 유저는 마지막 타격 후 2초 이내에 다음 블록을 파괴할 경우 콤보가 유지되어야 한다.
  1. 곡괭이가 블록을 파괴한다. `lastHitTime`이 기록된다.
  2. 2,000ms 이내에 다음 블록을 파괴한다.
  3. `now - lastHitTime < 2000` 조건 충족 → 콤보 카운터 1 증가.
  4. 배율이 업데이트된 콤보 단계 기준으로 재계산된다.

- **[STORY 8-8]** 유저는 마지막 타격 후 2초를 초과하면 콤보가 초기화되어야 한다.
  1. 마지막 블록 파괴 후 2,000ms 이상 경과한 뒤 다음 블록을 파괴한다.
  2. `now - lastHitTime >= 2000` 조건 → 콤보 카운터가 1로 리셋된다.
  3. 배율이 1.0x로 돌아간다.
  4. (참고: 콤보 초기화에 대한 별도 시각 알림 없음. 보상 금액이 낮아지는 것으로 체감.)

- **[STORY 8-9]** 유저가 여러 곡괭이를 동시에 운용하는 경우, 각 곡괭이의 콤보는 서로 독립적으로 계산되어야 한다.
  1. 플레이어 A가 곡괭이 X와 곡괭이 Y를 동시에 활성화한다.
  2. 곡괭이 X가 블록 10개를 연속 파괴한다 → 콤보 10, 배율 1.2x.
  3. 곡괭이 Y는 방금 배치되어 처음 블록 1개를 파괴한다 → 콤보 1, 배율 1.0x.
  4. 두 곡괭이의 콤보 상태(`combo`, `lastHitTime`)는 각 Pickaxe 인스턴스에 독립적으로 저장되므로 서로 영향을 주지 않는다.

- **[STORY 8-10]** 유저가 Hardcore 필드(10x 배율)에서 플레이할 경우, 콤보 배율과 필드 배율이 중첩 적용되어야 한다.
  1. 플레이어가 Hardcore 필드에서 곡괭이를 운용한다.
  2. 콤보 25 이상 달성 → 콤보 배율 1.5x.
  3. 필드 `rewardMultiplier`는 10.
  4. 최종 보상 = 블록 기본 보상 × 1.5(콤보) × 10(필드) = 기본 보상의 15배.
  5. 예시: 에메랄드(540cr) → 540 × 1.5 × 10 = 8,100cr.

- **[STORY 8-11]** 유저는 콤보 배율이 보상에 어떻게 적용되는지 계산 예시를 통해 이해할 수 있어야 한다.
  1. (인게임 직접 표시 없이 보상 수령 시 반영됨. 명세 참고용 예시.)
  2. 예시 A (Normal 필드, 콤보 없음): 철 블록(100cr) × 1.0 × 1 = 100cr.
  3. 예시 B (Normal 필드, 콤보 6): 철 블록(100cr) × 1.1 × 1 = 110cr.
  4. 예시 C (Normal 필드, 콤보 25+): 골드 블록(1,800cr) × 1.5 × 1 = 2,700cr.
  5. 예시 D (Hardcore 필드, 콤보 25+): 골드 블록(1,800cr) × 1.5 × 10 = 27,000cr.

---

### ❖ 3. 정책 및 데이터 정의

**COMBO 상수 (server/game/constants.js)**

| 항목 | 값 |
|------|-----|
| `COMBO.TIMEOUT` | 2,000ms (콤보 유지 시간 제한) |
| `COMBO.THRESHOLDS` | [0, 3, 6, 10, 15, 25] (콤보 단계 진입 기준) |
| `COMBO.MULTIPLIERS` | [1.0, 1.05, 1.1, 1.2, 1.35, 1.5] (각 단계 보상 배율) |
...
```
최종 보상 = round(블록 기본 보상 × 콤보 배율 × 필드 배율)
```

**구현 위치**
- `server/game/GameEngine.js` → `_getComboMultiplier(pickaxe, now)` 메서드
- 콤보 상태 저장: `Pickaxe` 인스턴스의 `combo`, `lastHitTime` 필드

---

### ❖ 4. 의존 영역 및 영향도

- **Pickaxe.js**: 각 인스턴스에 `combo` (정수), `lastHitTime` (timestamp ms) 필드 보유
- **GameEngine.js `_checkPickaxeCollisions`**: 블록 파괴 시 `_getComboMultiplier` 호출 후 `finalReward` 계산
- **constants.js COMBO**: 모든 배율·임계값의 단일 진실 소스. 이 파일 수정 시 전체 배율 체계 영향
- **Hardcore 필드**: `rewardMultiplier = 10` 이므로 콤보와 중첩 시 최대 15x 효과 발생
...
- **[STORY 9-1]** 유저는 잭팟 조건 미충족 상태에서 일반 플레이를 할 때 잭팟 블록이 등장하지 않아야 한다.
...
- **[STORY 9-2]** 유저는 잭팟 등장 조건 3가지가 모두 충족된 순간 0.05% 확률로 잭팟 블록이 등장할 수 있어야 한다.
...
- **[STORY 9-3]** 유저는 잭팟 블록이 등장했을 때 전체 알림을 확인할 수 있어야 한다.
...
- **[STORY 9-4]** 유저는 잭팟 블록을 곡괭이로 공략하여 피해를 줄 수 있어야 한다.
...
- **[STORY 9-5]** 유저는 TNT로 잭팟 블록을 공략할 수 있으나 40% 피해만 적용되어야 한다.
...
- **[STORY 9-6]** 유저는 잭팟 블록의 HP를 0 이하로 만들어 최종 파괴에 성공하면 250,000cr을 수령해야 한다.
...
- **[STORY 9-7]** 유저는 잭팟 블록 파괴 시 누가 파괴했는지 포함된 전체 알림을 확인할 수 있어야 한다.
...
- **[STORY 9-8]** 유저가 잭팟 블록을 파괴하지 못한 채 카메라가 블록 위치를 벗어나면 잭팟 블록이 소멸되어야 한다.
...
- **[STORY 9-9]** 유저는 잭팟이 파괴된 직후 누적 소비 카운터가 초기화되어 다음 잭팟까지 다시 1,500,000cr을 소비해야 함을 알아야 한다.
  1. 잭팟 블록이 파괴된다.
...
- **[STORY 10-1]** 유저는 퀘스트 패널을 열어 현재 진행 중인 퀘스트 목록을 볼 수 있어야 한다.
...
- **[STORY 10-3]** 미로그인 상태의 유저가 퀘스트 버튼을 클릭하면 로그인 유도가 표시되어야 한다.
...
- **[STORY 10-4]** 유저는 퀘스트 목록에서 각 퀘스트의 진행률 바를 확인할 수 있어야 한다.
  1. 퀘스트 패널이 열린다.
...
- **[STORY 10-5]** 유저가 블록을 파괴하면 관련 퀘스트의 진행 카운터가 증가해야 한다.
...
- **[STORY 10-7]** 유저는 "Complete On-Chain" 버튼을 클릭하여 지갑 서명 요청을 받아야 한다.
...
- **[STORY 10-8]** 유저가 트랜잭션에 서명하면 서버가 검증 중 상태가 되어야 한다.
...
- **[STORY 10-9]** 트랜잭션이 온체인에서 확인되면 퀘스트가 완료 처리되고 보상이 지급되어야 한다.
...
- **[STORY 10-10]** 유저가 지갑에서 트랜잭션을 거부하면 오류 처리가 되어야 한다.
...
- **[STORY 10-11]** 유저가 지갑 미연결 상태에서 온체인 완료 버튼을 클릭하면 연결 유도가 표시되어야 한다.
  1. 유저가 "Complete On-Chain" 버튼을 클릭한다.
  2. `window.WalletAPI.isConnected()` → false.
  3. 토스트: "Connect wallet first" 오류 메시지 표시.
  4. 버튼 상태 변화 없이 함수가 종료된다.

- **[STORY 10-12]** 이미 완료된 퀘스트를 재완료하려 하면 서버가 거부해야 한다.
...
- **[STORY 10-13]** 유저가 잘못된 네트워크(Sepolia 외)에서 트랜잭션을 보내면 실패 처리가 되어야 한다.
...
- **[STORY 10-14]** 유저는 Daily Player 퀘스트(매일 로그인)를 통해 로그인 일수에 따른 보상을 받을 수 있어야 한다.
...
- **[STORY 10-15]** 유저는 High Roller 퀘스트(총 획득 50,000cr)를 달성하여 보상을 받을 수 있어야 한다.
...
- **[STORY 11-1]** 유저는 리더보드 버튼을 클릭하여 순위 모달을 열 수 있어야 한다.
...
- **[STORY 11-9]** 현재 필드에 플레이어가 없을 때 리더보드는 "No players yet" 메시지를 표시해야 한다.
  1. `leaderboard` 배열이 빈 배열이거나 null인 경우.
  2. `UI.updateLeaderboard(leaderboard)`에서 빈 배열 감지.
  3. 컨테이너에 "No players yet" 텍스트가 표시된다 (회색, 중앙 정렬).

- **[STORY 11-10]** 유저가 Hardcore 필드로 이동하면 리더보드가 Hardcore 필드 플레이어만 표시해야 한다.
...
- **[STORY 12-1]** 유저는 채팅 버튼을 클릭하여 입력창을 열 수 있어야 한다.
...
- **[STORY 12-2]** 유저는 메시지 입력 후 Enter 키로 메시지를 전송할 수 있어야 한다.
...
- **[STORY 12-3]** 유저는 메시지 입력 후 전송 버튼 클릭으로 메시지를 전송할 수 있어야 한다.
  1. 유저가 채팅 입력창에 텍스트를 입력한다.
...
- **[STORY 12-9]** 유저는 같은 필드의 다른 플레이어가 보낸 메시지를 수신하여 채팅창에 표시되는 것을 확인할 수 있어야 한다.
...
- **[STORY 12-10]** 유저는 플레이어가 현재 필드에 입장했을 때 시스템 메시지를 확인할 수 있어야 한다.
...
- **[STORY 12-11]** 유저는 플레이어가 필드를 나갔을 때 시스템 메시지를 확인할 수 있어야 한다.
...
- **[STORY 12-12]** 채팅 메시지가 100개를 초과하면 가장 오래된 메시지가 자동으로 삭제되어야 한다.
